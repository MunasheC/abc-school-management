...
import com.bank.schoolmanagement.dto.GuardianResponse;
...
public class GuardianController {
     ...
     */
    @PostMapping
    public ResponseEntity<GuardianResponse> createGuardian(@Valid @RequestBody Guardian guardian) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(GuardianResponse.fromEntity(createdGuardian));
    ...
    public ResponseEntity<List<GuardianResponse>> getAllGuardians() {
        ...
        List<Guardian> guardians = guardianService.getAllGuardiansForCurrentSchool();
            List<GuardianResponse> dtos = guardians.stream()
                .map(GuardianResponse::fromEntity)
                .toList();
     ...
     */
    @GetMapping("/active")
    public ResponseEntity<List<GuardianResponse>> getActiveGuardians() {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<GuardianResponse> getGuardianById(@PathVariable Long id) {
    ...
    public ResponseEntity<GuardianResponse> getGuardianByPhone(@PathVariable String phone) {
     ...
     */
    @GetMapping("/search")
    public ResponseEntity<List<GuardianResponse>> searchGuardians(@RequestParam String name) {
    ...
    public ResponseEntity<List<GuardianResponse>> getGuardiansWithMultipleChildren() {
    ...
    public ResponseEntity<List<StudentResponse>> getChildrenForGuardian(@PathVariable Long id) {
        log.info("REST request to get children for guardian ID: {}", id);
        
        try {
            List<Student> children = guardianService.getChildrenForGuardian(id);
            List<StudentResponse> dtos = children.stream()
    ...
    public ResponseEntity<GuardianResponse> updateGuardian(
        ...
        try {
            ...
            return ResponseEntity.ok(GuardianResponse.fromEntity(updatedGuardian));
        } catch (IllegalArgumentException e) {
    ...
    public ResponseEntity<GuardianResponse> deactivateGuardian(@PathVariable Long id) {
        log.info("REST request to deactivate guardian with ID: {}", id);
        
        try {
            Guardian deactivated = guardianService.deactivateGuardian(id);
            return ResponseEntity.ok(GuardianResponse.fromEntity(deactivated));
        } catch (IllegalArgumentException e) {
    ...
    public ResponseEntity<GuardianResponse> reactivateGuardian(@PathVariable Long id) {
        log.info("REST request to reactivate guardian with ID: {}", id);
        
        try {
            Guardian reactivated = guardianService.reactivateGuardian(id);
            return ResponseEntity.ok(GuardianResponse.fromEntity(reactivated));
        } catch (IllegalArgumentException e) {
